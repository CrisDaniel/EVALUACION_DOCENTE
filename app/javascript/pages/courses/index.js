...
export default function Courses() {
  ...
  const [selectedCourse, setSelectedCourse] = React.useState(order[0])
  ...
  function handleGetCourses() {
    Axios({
      method: "get",
      url: `api/v1/courses?q=${filter.q}&order_by=${filter.order_by}&page=${filter.current_page}`,
      headers: {
    ...
    <section className="pt-10 h-full gap-6 pb-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-4">
          ...
          <Listbox value={selectedCourse} onChange={handleOrder} >
            {({ open }) => (
              <>
                <div className="relative">
                  <Listbox.Button className="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <span className="block truncate">
                      {selectedCourse.label == 'A-Z' ? 
                        <SortAscendingIcon
                  ...
                  { data.courses && (
                    ...
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Link to={`edit/${course.id}`} className="px-2 text-indigo-600 hover:text-indigo-900">Editar</Link>
                        <a type="button" onClick={() => handleDelete(course.id)} className="px-2 text-red-600 cursor-pointer hover:text-red-900">Eliminar</a>
                      </td>
                    </tr>
                    ))
                  )}
